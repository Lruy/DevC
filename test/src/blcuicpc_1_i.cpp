...
void pushdown(ll pos,ll l,ll r){
    if(!mp[pos].lazy && !(mp[pos].lazym-1))return;
    ll mid=(l+r)>>1;
    ll mul=mp[pos].lazym;
    ll val=mp[pos].lazy;
    mp[pos<<1].lazym*=mul;
...
void updatem(ll pos,ll l,ll r,ll L,ll R,ll val){
    if(L<=l && r<=R){
        mp[pos].lazym*=val;
        mp[pos].sum*=val;
        mp[pos].lazy*=val;
    ...
    pushdown(pos,l,r);
    ll mid=(l+r)>>1;
    if(L<=mid)updatem(pos<<1,l,mid,L,R,val);
    if(R>mid)updatem(pos<<1|1,mid+1,r,L,R,val);
    pushup(pos);
}
